		...
		return new Promise ((resolve,reject)=>{
			try {
				...
				this.socket.on("data",(buffer)=>{
					...
					//Get Cseq headers
					const cseqs = response.headers.find(header=>header.name=="Cseq");
						
					//Try get it from headers or first in transaction queue
					const cseq = cseqs.length ? cseqs.values[0].value : Object.keys(this.transactions)[0];
					...
					if (response.statusCode==401 && !transaction.authenticated && this.authenticator)
					{
						...
						for (let header of headers.values)
						{
							...
							if (header.value.startsWith("Digest "))
							{
								...
								transaction.request.headers = transaction.request.headers.filter(header => header.name!="Authorization");
								//Push it
